...
class MoodTracker:
    """Automated mood tracking system with periodic emotion capture and analysis"""
    ...
    def start_tracking(self, show_preview=False):
        ...
        if self.model is None:
            print("❌ Cannot start tracking: Model not loaded")
            return
    ...
    def load_mood_history(self, filename="mood_history.json"):
        """Load mood history from file"""
        try:
            if os.path.exists(filename):
                with open(filename, 'r') as f:
                    self.mood_history = json.load(f)
                print(f"📂 Mood history loaded from {filename}")
                return True
        except Exception as e:
            print(f"❌ Failed to load mood history: {e}")
        return False
...
def main():
    """Main function to demonstrate mood tracking"""
    print("🎭 Automated Mood Tracker")
    print("="*40)
...
class MoodTrackerAPI:
    """API for integrating mood tracker with other applications"""
    ...
    def get_current_mood(self):
        """Get current mood - this is the parameter you requested"""
        return self.tracker.get_current_mood()
    
    def get_mood_summary(self):
        """Get summary of recent moods"""
        history = self.tracker.get_mood_history()
        if not history:
            return None
        
        recent_moods = history[-5:]  # Last 5 mood analyses
        ...
        return {
            ...
            'recent_dominant': dominant_mood,
            'recent_count': len(recent_moods),
...
if __name__ == "__main__":
    ...
    # Uncomment below to test API integration
    # api = MoodTrackerAPI()
